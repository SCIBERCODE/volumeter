...
template <class Numeric, class Coeff = float_type> class iir {
 ...
 public:
  ...
  void process(const std::vector<Numeric>& in, std::vector<Numeric>& out) {
    for (size_t j = 0; j < in.size(); j++) out[j] = clock(in[j]);
  }
  void process(const std::vector<float>& in, std::vector<double>& out) {
    for (size_t j = 0; j < in.size(); j++) out[j] = (double)clock((Numeric)in[j]);
  }
  void process(const std::vector<double>& in, std::vector<float>& out) {
    for (size_t j = 0; j < in.size(); j++) out[j] = (float)clock((Numeric)in[j]);
  }
  void process_inplace(std::vector<Numeric>& io) {
    for (size_t j = 0; j < io.size(); j++) io[j] = clock(io[j]);
  }
  void process_inplace(std::vector<float>& io) {
    for (size_t j = 0; j < io.size(); j++) io[j] = (float)clock((Numeric)io[j]);
  }
#ifdef USE_FLOAT
  void process_inplace(std::vector<double>& io) {
    for (size_t j = 0; j < io.size(); j++) io[j] = (double)clock((Numeric)io[j]);
  }
#endif
};
}  // namespace spuce
